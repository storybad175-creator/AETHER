...
def decode_player_data(raw_encrypted: bytes) -> PlayerData:
    ...
    Handles recursive protobuf decoding and epoch to ISO string conversion.
    """
    try:
        # Step 1: AES Decrypt
        try:
        ...
        # Strategy B now handles recursion for field IDs 1-9
        raw_msg = decode_response(decrypted_bytes)

        # If decode_response returns something that doesn't look like a dict with keys,
        # it might be a decryption failure that didn't trigger an error (e.g. wrong key but padding valid)
        if not isinstance(raw_msg, dict) or not any(k in raw_msg for k in range(1, 10)):
             raise FFError(
                ErrorCode.DECODE_ERROR,
                "Protobuf decoding failed to find expected root fields.",
                extra={"possible_key_rotation": True, "action": "Update AES_KEY and AES_IV in .env"}
            )
        ...
        def decode_nested(data: Any) -> Dict[int, Any]:
            """Helper to ensure we have a dict for nested mapping, even if Strategy B didn't recurse."""
