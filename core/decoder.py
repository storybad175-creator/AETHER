...
def decode_player_data(raw_encrypted: bytes) -> PlayerData:
    ...
    Utilizes recursive Strategy B decoding from core.proto.
    """
    try:
        # Step 1: AES Decrypt
        ...
        account = {
            ...
            "language": to_str(safe_get(acc_raw, fields.FLD_ACC_LANG)) or "English",
            "signature": to_str(safe_get(acc_raw, fields.FLD_ACC_SIGNATURE)),
            "honor_score": safe_get(acc_raw, fields.FLD_ACC_HONOR),
        ...
        def parse_stat_line(data: Any) -> Dict[str, Any]:
            if not isinstance(data, dict):
                # Fallback if it wasn't decoded recursively (e.g. if proto.py logic failed)
                if isinstance(data, bytes) and data:
                    data = decode_response(data)
                else:
                    data = {}

            m = safe_get(data, fields.FLD_SL_MATCHES, 0)
            w = safe_get(data, fields.FLD_SL_WINS, 0)
            k = safe_get(data, fields.FLD_SL_KILLS, 0)
            de = safe_get(data, fields.FLD_SL_DEATHS, 0)
            hs = safe_get(data, fields.FLD_SL_HEADSHOTS, 0)
            ...
            return {
                ...
                "avg_damage_per_match": round(safe_get(data, fields.FLD_SL_AVG_DMG, 0.0), 2),
                "booyahs": safe_get(data, fields.FLD_SL_BOOYAHS, w)
            }
        ...
        stats = {
            ...
            "clash_squad": {
                "ranked": parse_stat_line(safe_get(stats_raw, fields.FLD_STATS_CS_RANKED))
            }
        }
        ...
        social = {
            "guild": {
                ...
                "leader": {
                    ...
                    "level": safe_get(social_raw, fields.FLD_SOC_GUILD_LEADER_LVL),
        ...
        pet = {
            ...
            "skin_id": safe_get(pet_raw, fields.FLD_PET_SKIN),
        ...
        cosmetics = {
            "avatar_id": safe_get(cosm_raw, fields.FLD_COSM_AVATAR),
            "banner_id": safe_get(cosm_raw, fields.FLD_COSM_BANNER),
            "pin_id": safe_get(cosm_raw, fields.FLD_COSM_PIN),
            "character_id": safe_get(cosm_raw, fields.FLD_COSM_CHAR),
            "equipped_outfit_ids": to_int_list(safe_get(cosm_raw, fields.FLD_COSM_OUTFITS, [])),
            "equipped_weapon_skin_ids": to_int_list(safe_get(cosm_raw, fields.FLD_COSM_WEAPONS, []))
        }

        # 7: Pass
        pass_raw = safe_get(raw_msg, fields.FLD_PASS_INFO, {})
        pass_info = {
            "booyah_pass_level": safe_get(pass_raw, fields.FLD_PASS_BP_LEVEL),
            "fire_pass_status": to_str(safe_get(pass_raw, fields.FLD_PASS_FP_STATUS)) or "Basic",
            "fire_pass_badge_count": safe_get(pass_raw, fields.FLD_PASS_FP_BADGES)
        }
        ...
        credit = {
            ...
            "reward_claimed": bool(safe_get(cred_raw, fields.FLD_CRED_REWARD)),
            "summary_period": to_str(safe_get(cred_raw, fields.FLD_CRED_SUMMARY))
        }

        # 9: Ban
        ban_raw = safe_get(raw_msg, fields.FLD_BAN, {})
        ban = {
            "is_banned": bool(safe_get(ban_raw, fields.FLD_BAN_STATUS)),
        ...
        def format_ts(epoch: Optional[int]) -> Optional[str]:
            if not epoch: return None
            return time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime(epoch))

        account["created_at"] = format_ts(account.get("created_at_epoch"))
        account["last_login"] = format_ts(account.get("last_login_epoch"))
