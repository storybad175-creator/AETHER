...
def decode_player_data(raw_encrypted: bytes) -> PlayerData:
    ...
    Utilizes the recursive output from core.proto.decode_response.
    """
    try:
        # Step 1: AES Decrypt
        decrypted_bytes = aes_decrypt(raw_encrypted)

        # Step 2: Protobuf Decode (Recursive Strategy B)
        ...
        def to_str(data: Any) -> Optional[str]:
            if isinstance(data, bytes):
                try:
                    ...
                    return None
            if isinstance(data, dict): # Should not happen for string fields
                return None
            return str(data) if data is not None else None

        def format_iso8601(epoch: Any) -> Optional[str]:
        ...
        # --- Sub-message Mapping (raw_msg is already recursive) ---
        ...
        account = {
            ...
            "created_at": format_iso8601(created_at_epoch),
            "last_login_epoch": last_login_epoch,
            "last_login": format_iso8601(last_login_epoch),
        ...
        stats = {
            ...
            "clash_squad": {
                "ranked": {
                    "matches": 0, "wins": 0, "win_rate": "0.00%", "kills": 0, "kd_ratio": 0.0
                }
            }
        }

        # CS Ranked specifically
