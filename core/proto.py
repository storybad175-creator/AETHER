...
try:
    ...
    HAS_COMPILED = False
    logger.info("Protoc compiled modules not found or incompatible. Using raw Strategy B parser.")
...
def decode_response_raw(data: bytes) -> Dict[int, Any]:
    """
    Decodes a response using recursive raw binary Strategy B.
    Automatically decodes nested messages for identified field IDs.
    """
    result = {}
    pos = 0

    # IDs of fields known to be nested messages
    ...
    while pos < len(data):
        ...
        if wire_type == 0: # Varint
            ...
            # Recursively decode if this is a known nested message ID
            if field_id in NESTED_IDS and len(val) > 0:
                try:
                    val = decode_response_raw(val)
                except Exception:
                    # If recursive decode fails, keep as raw bytes
                    pass
...
def decode_response(data: Union[bytes, Dict[int, Any]]) -> Dict[int, Any]:
    ...
    Accepts raw bytes or an already decoded dict to ensure consistency.
    """
