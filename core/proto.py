...
def decode_response_raw(data: bytes) -> Dict[int, Any]:
    ...
    Supports nested messages by returning a Dict[int, Any] recursively.
    """
    result = {}
    pos = 0
    ...
    while pos < len(data):
        ...
        if wire_type == 0: # Varint
            ...
            # Check if this field should be recursively decoded
            if field_id in MESSAGE_FIELDS:
                try:
                    val = decode_response_raw(val)
                except Exception:
                    # If it fails to decode as a message, keep it as raw bytes
                    pass
        ...
        # Store the value, handling repeated fields
