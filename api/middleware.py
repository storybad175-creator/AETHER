    ...
    async def _cleanup_loop(self):
        ...
        while True:
            ...
            async with self._lock:
                to_delete = [ip for ip, ts_list in self.visits.items()
            ...
            if len(self.visits[client_ip]) >= settings.RATE_LIMIT_RPM:
                return JSONResponse(
                    ...
                    content={
                        "error": {
                            "code": "RATE_LIMITED",
                            "message": f"Too many requests. Limit is {settings.RATE_LIMIT_RPM} RPM. Please try again later.",
                            "retryable": True
                        }
                    },
...
async def error_handler_middleware(request: Request, call_next):
    try:
        ...
        code = ErrorCode.INVALID_UID if "uid" in str(exc).lower() else ErrorCode.INVALID_REGION
        if "region" in str(exc).lower():
