...
# Error handler must be the outermost layer to catch everything
...
def find_available_port(start_port: int, max_attempts: int = 5) -> int:
    """Checks if a port is available, if not, tries the next one."""
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info(f"Server starting on port {final_port}")
